...
public class VideoUploadUseCase {
  ...
  public VideoUploadResponse execute(VideoUploadRequest request) {
    ...
    Video video = new Video(null, request.videoName(), request.format(), request.sizeInBytes(), user, LocalDateTime.from(Instant.now()), LocalDateTime.from(Instant.now()));
    ...
    videoRepositoryGateway.saveUrl(url);
